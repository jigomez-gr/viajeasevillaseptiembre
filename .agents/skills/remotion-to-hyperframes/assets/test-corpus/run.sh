...
run_render_tier() {
  ...
  if [[ -f "$fixture_dir/remotion-src/out/baseline.mp4" ]]; then
    echo "    ✓ using existing Remotion baseline"
  elif ! (cd "$fixture_dir/remotion-src" && \
  ...
  if [[ -f "$fixture_dir/hf.mp4" ]]; then
    echo "    ✓ using existing hf.mp4"
  elif ! (cd "$fixture_dir" && \
        npx hyperframes render hf-src/ --output hf.mp4 --quiet >/dev/null 2>&1); then
